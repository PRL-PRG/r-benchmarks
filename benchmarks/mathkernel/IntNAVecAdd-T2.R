# Element-wise add of two length-N integer vectors (10% NA), built-in + operator.
...
# vector length: one `A + B` over 10 million integers is far too short to measure
...
#
# Upstream built the vectors inside the timed kernel; setup() builds them into
# `.data` instead. They are not cached on disk (see harness.R).
...
execute <- function(rep = 15L) {
    cat("Vector Add two integer", .N, "size vectors(10% NA), built-in +, x", rep, "\n")
    A <- .data$A
    B <- .data$B
    for (k in 1:rep) {
        C <- A + B
