# Element-wise add of two length-N integer vectors (10% NA), iterative method.
...
execute <- function(rep = 1L) {
    cat("Vector Add two integer", .N, "size vectors(10% NA), iterative method, x", rep, "\n")
    A <- .data$A
    B <- .data$B
    for (k in 1:rep) {
        C <- vector("double", .N)
