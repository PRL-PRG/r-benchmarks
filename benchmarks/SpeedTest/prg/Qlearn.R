simulate <- function(init, world, gamma, alpha, epsilon, steps, n.states, n.actions) {
...
init2m = function (n.states) {
  marks <<- rep(0,n.states)
  sample(n.states,1)
}

world2m = function (s, a, n.states) {
...
execute <- function(size = 30000L) {
  ...
  result2m = simulate (function() init2m(n.states),
                       function(s, a) world2m(s, a, n.states),
                       gamma, alpha, epsilon, size, n.states, n.actions)
  res <- mean(result2m$history[,"r"]) # Average reward
  print(res)
  invisible(res)
}
