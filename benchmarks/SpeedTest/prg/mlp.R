mlp.forward <- function(X, q, params) {
  ...
  if (length(params) != (q+1) + q*(p+1)) {stop("Parameter vector is the wrong length")}
...
mlp.grad <- function(X, q, fw, bk) {
  ...
  for (j in 1:p) {dl.dgamma[j,] <- X[,j] %*% bk$dl.ds}
...
execute <- function(size = 200L) {
  ...
  res <- mlp.train (y, X, 0.004, 0.004, n.iters, 20)
  r1 <- round(res$ll[seq(1,n.iters,length=20)],2)
  r2 <- round(res$params[n.iters,],4)
