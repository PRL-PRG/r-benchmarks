kpca <- function(Xtrn, Xtst, rho, m) {
  ...
  for (t in 1:nrow(Xtst)) {
    k <- rep(NA, n)
    for (i in 1:n) {k[i] <- exp(-rho^2*sum((Xtrn[i,]-Xtst[t,])^2))}
    for (w in 1:m) {prj[t,w] <- (k - Ks) %*% M %*% e$vectors[,w] / sqrt(e$values[w])}
  }
  prj
}
...
execute <- function(size = 100L) {
  ...
  class.trn <- class[1:size]
  class.tst <- class[(size+1):(size)]

  for (i in 1:25) {prj <- kpca(X, X, 1, 3)}
  res <- coef(glm(class~prj,family="binomial"))
