...
# The size parameter is the number of shadow maps computed. It used to multiply a
# hard-coded ten of them, which made the shortest possible run 45 s - and every
# one of the ten is the same computation at the same sun angle, so nothing is lost
# by counting them directly. `execute(10)` is what `execute(1)` used to be.
execute <- function(n=1) {
  points <- rep(181L, n) # 181 takes the longest to compute
