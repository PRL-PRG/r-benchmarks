...
# Reading and parsing the 8-dimensional ALOI table is not the kernel: the
# clustering is. Decompressing and `read.csv`-ing it inside execute() put a
# gzip stream and the parser in the measured region.
setup <- function(n = 5) {
  alldata <- read.csv(gzfile("aloi-8d.csv.gz"), header = FALSE, sep = " ")
  .data <<- alldata[, 1:8]
  invisible()
}

execute <- function(n=5) {
  data <- .data

  set.seed(42)
  checksum <- flexclust(data, n)
  cat(n, " " , checksum, "\n")
  invisible(checksum)
}
